...
namespace PLIC_Web_Poratal.Controllers
{
    public class HomeController : Controller
    {
                    ...
                    using (SqlConnection conn = new SqlConnection(_db.GetConfiguration().GetConnectionString("DefaultConnection")))
                    {
...

                        {
                            ...
                            using (SqlDataReader reader = command.ExecuteReader())
                            {
                                // Create separate lists for the two charts
                                var chartDataAll = new List<ChartData>();
                                var chartDataComplaintsRegion = new List<ChartData>();
                                var chartDatComplaint = new List<ChartData>();
                                var chartDataInfo = new List<ChartData>();
                                var chartDataServiceRequest = new List<ChartData>();
                                var chartDataregion = new List<ChartData>();
                                ...
                                {
                                    var TotalTicket = (int)reader["TotalTicket"];
                                    var region = reader["regionname"].ToString();


                                    chartDataComplaintsRegion.Add(new ChartData
                                    {
                                        TotalTicket = TotalTicket,
                                        region = region,
                                ...
                                {
                                    chartDataAll = chartDataAll,
                                    chartDataComplaintsRegion = chartDataComplaintsRegion,
                                    chartDatComplaint = chartDatComplaint,
                                    chartDataInfo = chartDataInfo,
                                    chartDataServiceRequest = chartDataServiceRequest,
                                    chartDataregion = chartDataregion,
        ...
        [HttpGet]
        public async Task<ActionResult> GetTrackingDetails(string tracking, bool isCheckboxChecked)
        {
            try
            {
        ...
        //   update my below code apply asyn and await and also use using keywork in sql connection and not skip any of my code.

        public async Task<ActionResult> GetTicketDetails(string ticketno)
        {
            try
            {
                DataSet dataSet1 = new DataSet();
        ...
        public async Task<ActionResult> GetClaimDetailsbyClaimidAsync(string ticketno)
        {
            try
            {
                DataSet dataSet1 = new DataSet();
                  ...
                  //  using (SqlCommand command = new SqlCommand("sp_careconnect_Get_Tracking", conn))
...

                    {
                      ...
                      //  command.Parameters.AddWithValue("@bookingNumber", consignmentNumber);
                        ...
                        using (SqlDataAdapter dataAdapter5 = new SqlDataAdapter(command5))
                        using (SqlDataAdapter dataAdapter6 = new SqlDataAdapter(command6))
                        using (SqlDataAdapter dataAdapter7 = new SqlDataAdapter(command7))
                        {
                            await Task.Run(() =>
                            {
                                dataAdapter5.Fill(dataSet5);
        ...
        public async Task<ActionResult> GetTrackingGenerateDetailsClaim(string tracking)
        {
            try
            {
                DataSet dataSet = new DataSet();
                ...
                using (SqlConnection conn1 = new SqlConnection(_db.GetConfiguration().GetConnectionString("DefaultConnection")))
                {
                    ...
                    return PartialView("_TrackingGenerateDetailsClaim", model);
                }
            }
            catch (Exception ex)
            {
        ...
        public ActionResult GetSubcategoriesClaim(string category)
        {
        ...
        public async Task<IActionResult> SearchClaimAsync()
        {
            try
            {
                if (HttpContext.Session.GetString("LoginId") != "" && HttpContext.Session.GetString("LoginId") != null)
        ...
        public IActionResult ReportAgents()
        {
            // HttpContext.Session.Clear();
            // Route for Reneval Vouchers
            // changed by Aizaz and Imran Dated 17 Feb 22
            ...
            if (HttpContext.Session.GetString("LoginId") != "" && HttpContext.Session.GetString("LoginId") != null)
            {
                ...
                return View("~/Views/Home/ReportAgentsActivity.cshtml");
            }
        ...
        public async Task<IActionResult> TicketGenerate(string trackingNumber)
        {
            try
            {
                string password = HttpContext.Session.GetString("Password");
        ...
        public async Task<IActionResult> GenerateClaim(string trackingNumber)
        {
            try
            {
                string password = HttpContext.Session.GetString("Password");
                ...
                using (SqlConnection conn = new SqlConnection(_db.GetConfiguration().GetConnectionString("DefaultConnection")))
                {
                    if (HttpContext.Session.GetString("LoginId") != "" && HttpContext.Session.GetString("LoginId") != null)
                    {
                        ...
                        else
                        {
                            ...
                            return View("~/Views/Home/GenerateClaim.cshtml");
                        }
                    }
                }
        ...
        public async Task<ActionResult> CreateClaimAsync(CreateTicketModel createTicketModel, List<IFormFile> Images)
        {
            ...
            try
            {
                ...
                if (HttpContext.Session.GetString("LoginId") != "" && HttpContext.Session.GetString("LoginId") != null)
                {
                    using (SqlConnection conn = new SqlConnection(_db.GetConfiguration().GetConnectionString("DefaultConnection")))
                    {
                        ...
                        bool ticketExists = (bool)cmd.Parameters["@ticketExists"].Value;
                        int ticketID = (int)cmd.Parameters["@ClaimID"].Value;
                        string Complainer = (string)cmd.Parameters["@Complainer"].Value;
                        string ComplainerCell = (string)cmd.Parameters["@ComplainerCell"].Value;
                        ...
                        if (ticketExists)
                        {
                            ...
                            // Ticket already exists
                            return Json(new { success = false, message = "Claim already Created." });
                        }
                        else
                        {
                            if (createTicketModel.issendsms == false || createTicketModel.ticketcatagory == 6)
                                    ...
                                    if (objResponse.StatusCode == HttpStatusCode.OK)
                                    {
                                        using (StreamReader sr = new StreamReader(objResponse.GetResponseStream()))
                                        {
                                            ...
                                            if (statusCode == "200" || statusCode == "0")
                                            {
                                                ...
                                                // Ticket inserted successfully
                                                return Json(new { success = true, data = ticketupdateno, message = "Claim has been Created Successfully." });
                            ...
                            int ticketno = ticketID; // Replace with your tracking number variable or value
                            ViewData["TicketNo"] = ticketno;
                            return Json(new { success = true, data = ticketno, message = "Claim created successfully." });
                        }
                    }
                }
                return RedirectToAction("Login", "Account");
            ...
            catch (Exception ex)
            {
                ...
                return Json(new { success = false, message = "Error creating Claim.", error = ex.Message });
            }
        ...
        public async Task<IActionResult> SearchClaimDetailAsync(CreateTicketModel createTicketModel)
        {
            try
            {
                if (HttpContext.Session.GetString("LoginId") != "" && HttpContext.Session.GetString("LoginId") != null)
            ...
            {
                // Handle the exception and return an error response
                return Json(new { success = false, message = "Error Searching Claim.", error = ex.Message });
            }
        }
        [HttpGet]
        ...
        public IActionResult GetReportAgentsActivityDateWise(CreateTicketModel createTicketModel)
        {
            try
            {
                if (HttpContext.Session.GetString("LoginId") != "" && HttpContext.Session.GetString("LoginId") != null)
                {
                    using (SqlConnection conn = new SqlConnection(_db.GetConfiguration().GetConnectionString("DefaultConnection")))
                    {
                        SqlCommand cmd = new SqlCommand("sp_rpt_Agents_Activity", conn);
                        cmd.CommandType = CommandType.StoredProcedure;
                        ...
                        ViewBag.TrackingData = ds;
                        return PartialView("_AgentsActivityReportDetail", model);
                ...
                if (HttpContext.Session.GetString("LoginId") != "" && HttpContext.Session.GetString("LoginId") != null)
                {
                    using (SqlConnection conn = new SqlConnection(_db.GetConfiguration().GetConnectionString("DefaultConnection")))
                    {
                        ...
                        using (SqlCommand cmd = new SqlCommand("sp_Update_Claim_FinalRemarks", conn))
                        {
                            ...
                            cmd.Parameters.AddWithValue("@destination", updateTicketModel.destination);
                            cmd.Parameters.AddWithValue("@Activity", updateTicketModel.activity);
                            cmd.Parameters.AddWithValue("@isOrigin_Checked", updateTicketModel.isOrigin_Checked);
                            cmd.Parameters.AddWithValue("@is_Reject", updateTicketModel.is_Reject);
                            cmd.Parameters.AddWithValue("@isDestination_Checked", updateTicketModel.isDestination_Checked);
                            cmd.Parameters.AddWithValue("@Via_City_id", updateTicketModel.viaCityid);
                            ...
                            else
                            {
                                ...
                                // Ticket inserted successfully
                                return Json(new { success = true, data = ticketupdateno, message = "Claim Proceed Successfully." });
                            }
                        }
                        // string trackingNumbernew = name; // Replace with your tracking number variable or value
                        //ViewData["TrackingNumber"] = trackingNumbernew;
